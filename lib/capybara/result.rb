...
module Capybara
  class Result
    ...
    def failure_message
      ...
      if count > 0
        message << ", found #{count} #{declension("match", "matches")}: " <<
          @result.map{ |n| n.unsynchronized{ n.text } }.map(&:inspect).join(", ")
      else
        message << " but there were no matches"
      end
      unless @rest.empty?
        elements = @rest.map{ |n| n.unsynchronized{ n.text } }.map(&:inspect).join(", ")
